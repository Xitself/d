   ...
   if mouse.Target and uis:IsKeyDown(Enum.KeyCode.LeftControl) then
       ...
       for _, v in pairs(npcparts:GetDescendants()) do
           if v:IsA("BasePart") or v:IsA("Part") and v.Name ~= "HumanoidRootPart" then
               ...
               -- Create a connection for touch tap event
               touchTapConnection = userInputService.TouchTap:Connect(function(touchPositions)
                   -- Check if the user tapped on the screen
                   if touchPositions then
                       -- Get the part that was tapped
                       local tappedPart = mouse.Hit
                       if tappedPart then
                           -- Check if the tapped part is the same as the NPC part
                           if tappedPart == v then
                               v.CanCollide = true
                           end
                       end
                   end
               ...
               -- Clean up the connection when the script is finished
               wait(0)
